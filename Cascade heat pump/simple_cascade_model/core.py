...
from CoolProp import AbstractState, HmassP_INPUTS, PQ_INPUTS, QT_INPUTS, PSmass_INPUTS, PT_INPUTS, SmassT_INPUTS
from .constants import DISCRETIZATION, T_SUPERHEAT, P_AMB, MAX_ITER, TOLERANCE_ETAC, KELVIN, PENALTY
from .utils import get_air_temp_profile, EntropyProduction, EntropyProductionUpper
...
def UpperCycle(RefHigh, cycle, dryer, T_PinchInternal, T_PinchAir, etac, IHX_upper, T_start_point, PrintData, HTF, EntropyUpper):
    ...
    if HTF:
        ...
        cycle.htf["T_sink"] = np.array([HtfUpper.update(HmassP_INPUTS, h, P_AMB) or HtfUpper.T() for h in cycle.htf["h_sink"]])
    ...
    for _ in range(MAX_ITER):
        ...
        try:
            ...
            p_max = 2 * P_crit
        res_high = minimize_scalar(objective_high, bounds=(cycle.high['P2'], p_max),
                                   method='bounded', options={'maxiter': MAX_ITER, 'xatol': TOLERANCE_ETAC})
        ...
        for s, T in zip(s_vals, T_vals):
            try:
                ...
                Q = RefHigh.Q()
                if 0 < Q < 1:
                    # print(f"Wet compression detected at T = {T - KELVIN:.2f} C , kJ/kg, Q = {Q:.3f}")
                    wet_detected = True
                    break
            except ValueError:
                continue
        ...
        if wet_detected:
            cycle.high['T2'] += 1  # Raise inlet temperature to avoid wet compression
            T2_increased = True
        ...
        if cycle.high['P4'] > P_crit:
            if cycle.high['s4'] > s_crit:
                T_CondIn += 1  # Increase temperature condenser in
                continue
        else:
            RefHigh.update(PQ_INPUTS, cycle.high['P3'], 0)
            T_bubble = RefHigh.T()
            if cycle.high['T4'] > T_bubble:
                T_CondIn += 1  # Increase condenser pressure
                continue

        # --- Pinch check in condenser ---
        cycle.high['h_Cond'] = np.linspace(cycle.high['h3'], cycle.high['h4'], N)
        cycle.high['T_Cond'] = np.array([RefHigh.update(HmassP_INPUTS, h, cycle.high['P3']) or RefHigh.T() for h in cycle.high['h_Cond']])
        ...
        if IHX_upper:
            ...
            # --- Check pinch point in IHX ---
            cycle.high['h_IHX_high'] = np.linspace(cycle.high['h4'], cycle.high['h5'], N)
            cycle.high['T_IHX_high'] = np.array([RefHigh.update(HmassP_INPUTS, h, cycle.high['P4']) or RefHigh.T() for h in cycle.high['h_IHX_high']])
            
            cycle.high['h_IHX_low'] = np.linspace(cycle.high['h1'], cycle.high['h2'], N)
            cycle.high['T_IHX_low'] = np.array([RefHigh.update(HmassP_INPUTS, h, cycle.high['P2']) or RefHigh.T() for h in cycle.high['h_IHX_low']])
            ...
            if Delta_T_IHX < 0:
                if T2_increased:
                    ...
                    cycle.high['T2'] -= 1  # Lower inlet temperature to make IHX posible
                    RefHigh.update(PT_INPUTS, cycle.high['P2'], cycle.high['T2'])
                    cycle.high['s2'] = RefHigh.smass()
                    cycle.high['h2'] = RefHigh.hmass()
                    cycle.high['T2'] = RefHigh.T()
                ...
                # --- Temperature profile in shared HEX ---
                cycle.high['h_Evap'] = np.linspace(cycle.high['h6'], cycle.high['h1'], N)
                cycle.high['T_Evap'] = np.array([RefHigh.update(HmassP_INPUTS, h, cycle.high['P2']) or RefHigh.T() for h in cycle.high['h_Evap']])
        else:
            # --- Point 5 (expansion) ---
    ...
    # --- Mass flow refrigerant upper cycle ---
    delta_h = cycle.high['h3'] - cycle.high['h4']
    cycle.high['m_dot'] = dryer.Q_heating / delta_h
    
    if PrintData:
        print(f'In the upper cycle the superheat temperature is {cycle.high["T2"]-(T_start_point + KELVIN)} K,\
the pressure ratio is {cycle.high['P3']/cycle.high['P2']:.2f}, the pressure compressor out is {cycle.high['P3']/1e5:.2f} bar \
and the mass flow is {cycle.high['m_dot']:.3f} kg/s')
    if EntropyUpper:
        return EntropyProductionUpper(cycle, dryer, IHX_upper, HTF), cycle.high["P3"]/cycle.high["P2"], cycle.high["P3"]
    else:
        return IHX_upper
    
def LowerCycle(RefLow, cycle, dryer, T_PinchInternal, T_PinchAir, etac, IHX_upper, IHX_lower, PrintData, HTF):
    ...
    for _ in range(MAX_ITER):
        ...
        p_max = RefLow.p()
        res_low = minimize_scalar(objective_low, bounds=(cycle.low['P6'], p_max),
                                   method='bounded', options={'maxiter': MAX_ITER, 'xatol': TOLERANCE_ETAC})
        ...
        if cycle.low['P8'] > P_crit:
            ...
            if cycle.low['T9'] > T_bubble:
                T_CondIn += 1  # Increase condenser inlet temperature
                continue
        ...
        if wet_detected:
            T_super += 1  # Raise inlet temperature to avoid wet compressio35.89n
            T7_increased = True
            continue
        ...
        if IHX_lower:
            ...
            cycle.air['T_source'] = get_air_temp_profile(cycle.air['h_source'], dryer.pt4.W, dryer.pt4.H, 1, P_AMB)
            cycle.air['s5'] = HAPropsSI("S", "H", cycle.air["h_source"][-1], "T", cycle.air["T_source"][-1], "P", P_AMB)
            ...
            if HTF:
                ...
                cycle.htf["T_source"] = np.array([HtfLower.update(HmassP_INPUTS, h, P_AMB) or HtfLower.T() for h in cycle.htf["h_source"]])
            
            # --- Pinch check in evaporator ---
            cycle.low["h_Evap"] = np.linspace(cycle.low['h11'], cycle.low['h6'], N)
            cycle.low["T_Evap"] = np.array([RefLow.update(HmassP_INPUTS, h, cycle.low['P7']) or RefLow.T() for h in cycle.low["h_Evap"]])
            ...
            cycle.low["T_Cond"] = np.array([RefLow.update(HmassP_INPUTS, h, cycle.low['P8']) or RefLow.T() for h in cycle.low["h_Cond"]])
            ...
            cycle.low['T_IHX_high'] = np.array([RefLow.update(HmassP_INPUTS, h, cycle.low['P9']) or RefLow.T() for h in cycle.low['h_IHX_high']])
            
            cycle.low['h_IHX_low'] = np.linspace(cycle.low['h6'], cycle.low['h7'], N)
            cycle.low['T_IHX_low'] = np.array([RefLow.update(HmassP_INPUTS, h, cycle.low['P6']) or RefLow.T() for h in cycle.low['h_IHX_low']])
            ...
            if Delta_T_IHX < 0:
                if T7_increased:
                    ...
                    T_super -= 1  # Lower inlet temperature compressor to make IHX posible
    ...
    if PrintData:
        print(f'In the lower cycle the superheat temperature is {cycle.low["T7"]-cycle.low["T6"]} K,\
the pressure ratio is {cycle.low['P8']/cycle.low['P7']:.2f}, the pressure compressor out is {cycle.low['P8']/1e5:.2f} bar \
and the mass flow is {cycle.low['m_dot']:.3f} kg/s')
        print(f'The air temperature out of the evaporator is {cycle.air['T_source'][-1]-KELVIN:.2f} C') 
